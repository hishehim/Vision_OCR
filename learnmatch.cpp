...
*/
void processFeatures(vector<Mat> images, BOWKMeansTrainer &bowTrainer, Ptr<DescriptorExtractor> extractor, SurfFeatureDetector detector){
...
void dataTraining (vector<Mat> images, Mat &labels, Mat &trainingData, SurfFeatureDetector detector, BOWImgDescriptorExtractor &bowDE, char character) {
	vector<KeyPoint> keypoints;
	Mat bowDescriptor;
	for (int j=0; j<images.size(); j++) {
		Mat image = images.at(j);
	...
	*/
	...
	if (run==1) {
		...
		*/
		...
		Mat fileImage1 = imread(argv[1],CV_LOAD_IMAGE_GRAYSCALE);
		Mat dictionary1 = fileImage1>threshold;

		Mat fileImage2 = imread(argv[2],CV_LOAD_IMAGE_GRAYSCALE);
		Mat dictionary2 = fileImage2>threshold;
		
		Mat fileImage3 = imread(argv[3],CV_LOAD_IMAGE_GRAYSCALE);
		Mat dictionary3 = fileImage3>threshold;
		
		cout<<trainingImages.at(0).size()<<endl;
		
		/********** ********** ********** ********** **********
		 Emily-Mike Code goes Here
...
//			}
//		}
//		for(int i = 0; i<trainingImages.size(); i+=imageCount) {
//			vector<Mat> images;
//			for (int j=0; j<imageCount; j++) {
//				int index = (i+j)-1;
//				Mat fileImage = imread(argv[index],CV_LOAD_IMAGE_GRAYSCALE);
...
//		*/
//		
//		for(int i = 1; i<argc-2; i+=imageCount) {
//			vector<Mat> images;
...
//		detectedText.open(detectedTextFilePath);
//		CvSVM svm;
...
//		Mat fileImage = imread(argv[4], CV_LOAD_IMAGE_GRAYSCALE);
//		Mat image = fileImage>threshold;
